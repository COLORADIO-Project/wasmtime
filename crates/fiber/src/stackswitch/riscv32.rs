...
unsafe extern "C" fn wasmtime_fiber_switch_(top_of_stack: *mut u8 /* a0 */) {
    naked_asm!(
      ...
      sw s10,-0x30(sp)
      sw s11,-0x34(sp)
      addi sp , sp , -0x40 // Must at least change by 0x38, choose 0x40 to be 16-byte aligned
      ...
      lw s11,0xc(sp) // The offset starts with 0xc because of the unused space from the alignment above
      lw s10,0x10(sp)
      lw s9,0x14(sp)
      lw s8,0x18(sp)
      lw s7,0x1c(sp)
